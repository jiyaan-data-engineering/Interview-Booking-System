...
export default function TodayScheduleTab({ slots }: TodayScheduleTabProps) {
  ...
  const filteredTodaySlots = todayConfirmedSlots.filter(slot => {
  ...
  // Get all time conflicts
  const getInviteStatusColor = (interviewStatus: string | undefined) => {
    if (interviewStatus === 'Received') return 'bg-green-600 text-green-100';
    if (interviewStatus === 'Waiting for Invite') return 'bg-yellow-600 text-yellow-100';
  ...
  const getTimeConflicts = () => {
    const conflicts: {
      ...
      candidates: string[];
      type: string;
      details: { name: string; startTime: string; endTime: string; duration: string; room: string; interviewStatus: string | undefined }[]
    }[] = [];
    const processed = new Set<string>();

    for (let i = 0; i < todayConfirmedSlots.length; i++) {
      for (let j = i + 1; j < todayConfirmedSlots.length; j++) {
        const slot1 = todayConfirmedSlots[i];
        const slot2 = todayConfirmedSlots[j];
            ...
            details: [
              {
                ...
                room: slot1.room || 'N/A',
                interviewStatus: slot1.interviewStatus
              },
              {
                ...
                room: slot2.room || 'N/A',
                interviewStatus: slot2.interviewStatus
              }
            ]
          });
        }
  ...
  // Get current time and show only upcoming interviews
  ...
  const upcomingSlots = todayConfirmedSlots.filter(slot => {
    ...
    <div>
      ...
      {upcomingSlots.length === 0 ? (
        <div className="text-center py-8 bg-slate-800/50 rounded-lg">
          ...
          <p className="text-slate-400">No upcoming interviews for today</p>
        </div>
      ) : (
        <>
          {todayConfirmedSlots.length > 0 && (
            <div className="mb-6 p-3 bg-slate-800 rounded-lg border border-slate-700">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
                {Array.from(candidateCount.entries()).map(([name, count]) => {
                  const candidateCompanies = [...new Set(todayConfirmedSlots.filter(s => s.candidateName === name).map(s => s.company))];
                  return (
          ...
          {filteredTodaySlots.filter(s => timeToMinutes(s.time) >= currentTimeInMinutes).length === 0 ? (
            ...
            <div className="space-y-4">
              {filteredTodaySlots.filter(s => timeToMinutes(s.time) >= currentTimeInMinutes).map((slot, index) => {
                ...
                <div key={slot.id} className="slot-card">
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-4">
                    ...
                    {slot.supportPerson && (
                      <div>
                        <div className="text-slate-400 text-sm font-semibold mb-1">👨‍💼 Support Person</div>
                  ...
                  {slotConflicts.length > 0 && (
                    <div className="mt-4 p-4 bg-red-900/30 border border-red-500/50 rounded-lg mb-4">
                              ...
                              <div className="ml-4 space-y-1">
                                {conflict.details.map((detail, dIdx) => (
                                  <div key={dIdx} className="text-red-100 text-xs bg-slate-900/50 p-2 rounded">
                                    ...
                                    <div className="text-red-300 text-xs">🚪 Intrivew at : {detail.room}</div>
                                    {detail.interviewStatus && (
                                      <div className={`text-xs px-2 py-0.5 rounded mt-1 inline-block ${getInviteStatusColor(detail.interviewStatus)}`}>
                                        📋 {detail.interviewStatus}
                                      </div>
                                    )}
                                  </div>
                  ...
                  {slot.feedback && (
                    <div className="mb-2 p-2 bg-slate-700/50 rounded border border-slate-600">
                      <div className="text-xs text-slate-400">Feedback: <span className="text-white font-semibold">{slot.feedback}</span></div>
                    </div>
                  )}

                  {slot.comments && (
                    <div className="p-2 bg-slate-700/50 rounded border border-slate-600">
                      <div className="text-xs text-slate-400">Comments: <span className="text-white font-semibold">{slot.comments}</span></div>
                    </div>
                  )}
                </div>
              );
